...
LastIt   = ["7200", "7200", "7200", "7200", "7200", "7200", "7200"]
...
for i, pat in enumerate(patterns[0:-2]):
    print("Dealing with", pat+"_base", "and", patterns[i+2])
    tmpFile, abs_path = mkstemp()
    with open(tmpFile,'w') as new_file:
        with open("inputs.compare", 'r') as old_file:
            for line in old_file:
                if (searchFor1 in line):
                    new_file.write("compare.exactRoot =  ../"+patterns[i+2]+"/plot00"+LastItSL[i+2]+".2d.hdf5")
                elif (searchFor2 in line): 
                    new_file.write("compare.computedRoot =  ../"+pat+"_base/plot00"+LastIt[i]+".2d.hdf5")
                else:
                    new_file.write(line)
    tmp_file_loc = convergence_dir+"/tmp/inputs.compare_"+pat+"_base-"+patterns[i+2]
    print("... moving input file to", tmp_file_loc)
    shutil.move(abs_path, tmp_file_loc)
    os.system("/{} {}".format(executable, tmp_file_loc))
    shutil.move("pout.0", "./tmp/ERR_"+pat+"_base-"+patterns[i+2])
    print("... moving outpout file to", convergence_dir+"/tmp/ERR_"+pat+"_base-"+patterns[i+2])
...
for i, pat in enumerate(patterns[0:-2]):
    ...
    with open(tmpFile,'w') as new_file:
        with open("inputs.compare_custom", 'r') as old_file:
            for line in old_file:
                if (searchFor1 in line):
                    new_file.write("compare.exactRoot =  ../"+patterns[i+2]+"/plotCustom00"+LastItSL[i+2]+".2d.hdf5")
                elif (searchFor2 in line): 
                    new_file.write("compare.computedRoot =  ../"+pat+"_base/plotCustom00"+LastIt[i]+".2d.hdf5")
                else:
                    new_file.write(line)
    tmp_file_loc = convergence_dir+"/tmp/inputs.compare_custom_"+pat+"_base-"+patterns[i+2]
    print("... moving input file to", tmp_file_loc)
    shutil.move(abs_path, tmp_file_loc)
    os.system("/{} {}".format(executable, tmp_file_loc))
    shutil.move("pout.0", "tmp/ERR_"+pat+"_base-"+patterns[i+2]+"_custom")
    print("... moving outpout file to", convergence_dir+"/tmp/ERR_"+pat+"_base-"+patterns[i+2]+"_custom")
