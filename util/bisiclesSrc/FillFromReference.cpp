                       ...
                       CoarseAverage::averageType a_avgType)
{
  ...
  if (refRatio > 1+tolerance) {
      ...
      if (ghostVect[0] != 0) {
          ...
          pwl.fillInterp(a_destData, srcLD, srcLD, time_interp_coeff, 0, 0, 1);
      ...
      int nRef = (int) (refRatio + tolerance);
      if (a_verbose) {
          pout() << " ...averaging data with refinement ratio = " << nRef << endl;
      }
      CoarseAverage averager(srcDBL,destGrids, 1, nRef, a_destData.ghostVect());
