                          ...
                          Vector<LevelData<FArrayBox>*>& a_RHS)
{
    ...
    Vector<RefCountedPtr<LevelData<FArrayBox> > > mask(m_finest_level + 1);
    ...
    for (int lev = 0; lev <= m_finest_level; lev++) {
        ...
        mask[lev]  = RefCountedPtr<LevelData<FArrayBox> >(new LevelData<FArrayBox>(m_amrGrids[lev], 1, nGhost));
        ...
        for (dit.begin(); dit.ok(); ++dit) {
            ...
            (*mask[lev])[dit].copy(levelIceMask[dit], 0, 0, 1);
        }
    }
    ...
    poissonOpF_head.define(a_domains[0],
                          ...
                          PDfunctTmp,
                          B, Pri, zb, mask, m_compute_Bcoeff);
...
AmrHydro::compute_grad_var(int lev, 
                           Vector<RefCountedPtr<LevelData<FArrayBox> > >& var,
                           LevelData<FArrayBox>& levelGradvar)
{
    // Compute grad(var) -CC- 
    DisjointBoxLayout& levelGrids       = m_amrGrids[lev];
    LevelData<FArrayBox>& levelvar      = *var[lev];    

    DataIterator dit = levelGradvar.dataIterator();
    for (dit.begin(); dit.ok(); ++dit) {
        FArrayBox& thisvar = levelvar[dit];
        FArrayBox& thisGrad = levelGradvar[dit];
        ...
        for (bit.begin(); bit.ok(); ++bit) {
            ...
            for (int dir = 0; dir < SpaceDim; dir++) {
                ...
                thisGrad(iv, dir) = 0.5 * (thisvar(plusIV, 0) - thisvar(minusIV, 0)) / m_amrDx[lev][dir];
            }
            // replace with magnitude
            thisGrad(iv, 0)  = std::sqrt(  thisGrad(iv, 0)*(iv, 0)
                                           + thisGrad(iv, 1)*thisGrad(iv, 1) );
        } // end loop over cells
    }     // end loop over grids
}
...
AmrHydro::compute_grad_head(int lev)
{
    ...
    LevelData<FArrayBox>& levelgradH    = *m_gradhead[lev];    // GC
    LevelData<FluxBox>&   levelgradH_ec = *m_gradhead_ec[lev]; // No GC
...
void
AmrHydro::timeStepFAS(Real a_dt, bool do_head)
{
    ...
    Vector<LevelData<FArrayBox>*> a_gradIM;
    // DEBUG 
    ...
    a_gradIM.resize(m_finest_level + 1, NULL);
    ...
    Vector<LevelData<FluxBox>*> a_Qw_ec_GC;
    ...
    a_Qw_ec_GC.resize(m_finest_level + 1, NULL);
    ...
    Vector<RefCountedPtr<LevelData<FluxBox> > > bCoef_fluxes(m_finest_level + 1);
    ...
    for (int lev = 0; lev <= m_finest_level; lev++) {
        ...
        if (lev > 0) {
            int nGhost = currentB.ghostVect()[0];
        ...
        a_gradIM[lev]             = new LevelData<FArrayBox>(m_amrGrids[lev], 1*SpaceDim, HeadGhostVect);
        ...
        bCoef_fluxes[lev] = RefCountedPtr<LevelData<FluxBox> >(new LevelData<FluxBox>(m_amrGrids[lev], 1, IntVect::Unit));
        // Face centered stuff
        a_Qw_ec[lev]         = new LevelData<FluxBox>(m_amrGrids[lev], 1, IntVect::Zero);
        a_Qw_ec_GC[lev]      = new LevelData<FluxBox>(m_amrGrids[lev], 1, IntVect::Unit);
        ...
        //m_IBCPtr->resetCovered(levelDx, *m_suhmoParm, *m_head[lev], *m_gapheight[lev], *m_overburdenpress[lev]);
        LevelData<FArrayBox>& levelGradPhi = *a_gradIM[lev];
        compute_grad_var(lev, m_iceMask, levelGradPhi); 
    ...
    while (!converged_h) { 
        ...
        for (int lev = 0; lev <= m_finest_level; lev++) {
            ...
            //m_IBCPtr->resetCovered(*m_suhmoParm, *m_head[lev], *m_iceheight[lev], *m_overburdenpress[lev]);

        }  // loop on levs -- same thing, we should start with consistent b and h GC/BC and all


        //         Compute grad(h) and grad(zb) and lazy diff coef
        if (m_verbosity > 3) {
        ...
        for (int lev = 0; lev <= m_finest_level; lev++) {
            ...
            // Compute grad(Zb) -EC-
            LevelData<FluxBox>& levelgradZb_ec  = *a_gradZb_ec[lev]; // No GC for this variable
        ...
        for (int lev = 0; lev <= m_finest_level; lev++) {
            ...
            Calc_meltingRate(lev, leveltmp_cc, leveltmp2_cc, levelMR_A, levelMR_B, levelMR_C); 
        ...
        for (int lev = 0; lev <= m_finest_level; lev++) {
            ...
            LevelData<FluxBox>& levelbcoefF = *bCoef_fluxes[lev];
            LevelData<FluxBox>& levelRe_ec  = *a_Re_ec[lev]; 
            LevelData<FluxBox>& levelB_ec   = *a_GapHeight_ec[lev];    
            LevelData<FluxBox>& levelPi_ec  = *a_Pi_ec[lev];    
            ...
            for (dit.begin(); dit.ok(); ++dit) {
                ...
                levelbcoefF[dit].copy(levelbcoef[dit], 0, 0, 1);
            }
            CopyGhostCellsEC(levelbcoefF,  m_amrDomains[lev]);
        } // loop on levs
        ...
        if (ite_idx > 100) {
            ...
            if (m_cur_step < 2) { // precaution for screwed up initialization
                ...
                if (m_regrid) {
                    Real eps_tmp = m_eps_PicardIte*10.0;  
                    if (max_resH < eps_tmp){
                        if (m_verbosity > 0) {
                            pout() <<"        converged( it = "<< ite_idx+1 << ", x(h) = " <<max_resH<< ")."<< endl;
                        }
   ...
   if (m_verbosity > 3) {
    ...
    for (int lev = 0; lev <= m_finest_level; lev++) {
        ...
        // EC quantities
        LevelData<FluxBox>& levelQw_ec      = *a_Qw_ec[lev]; 
        LevelData<FluxBox>& levelQw_ec_GC   = *a_Qw_ec_GC[lev];
        LevelData<FluxBox>& levelB_ec       = *a_GapHeight_ec[lev];    
        evaluate_Qw_ec(lev, levelQw_ec, levelB_ec, levelRe_ec);
        EdgeToCell(levelQw_ec, levelQw); 
        // handle ghost cells on the coarse-fine interface
        ...
        for (dit.begin(); dit.ok(); ++dit) {
            ...
            /* for PP... need a place to do so */
            levelQw_ec_GC[dit].copy(Qwater_ec, 0, 0, 1);
        }
        CopyGhostCellsEC(levelQw_ec_GC,  m_amrDomains[lev]);
        // Qw gradH
        EdgeToCell(leveltmp_ec,  leveltmp_cc);
        // Qw gradZb
        EdgeToCell(leveltmp2_ec, leveltmp2_cc);
    ...
    if ((m_PrintCustom) && (m_cur_step % m_plot_interval == 0) ) {
        ...
        for (int lev = 0; lev <= m_finest_level; lev++) {
            ...
            LevelData<FArrayBox>& levelCD        = *a_gradIM[lev];
    ...
    for (int lev = m_finest_level; lev > 0; lev--) {
        ...
        //averager.averageToCoarse(*m_head[lev - 1], *m_head[lev]); -- done in Picard loop

        CoarseAverageFace averagerEC(m_amrGrids[lev], 1, m_refinement_ratios[lev - 1]);
        averagerEC.averageToCoarse(*bCoef_fluxes[lev - 1], *bCoef_fluxes[lev]);
    ...
    if (m_post_proc) {
        pout() << "\n\n\n";
        pout() <<"oo POST PROC analysis (domsize X/Y "<< m_amrDomains[0].domainBox().size(0) << ", " <<  m_amrDomains[0].domainBox().size(1)<< ") oo "<< endl;
 
        if (m_post_proc_comparisons) {
            interpVelData();
        }

        check_fluxes(bCoef_fluxes);
        ...
        Vector<Real> out_recharge_MR(DomSize, 0.0);
        Vector<Real> out_recharge_ext(DomSize, 0.0);
        ...
        LevelData<FluxBox>&   levelQw_ec    = *a_Qw_ec_GC[0]; 
        ...
        for (dit.begin(); dit.ok(); ++dit) {
            ...
            /* EC */
            BoxIterator bitEC(CD_ecFab.box());
            for (bitEC.begin(); bitEC.ok(); ++bitEC) {
                IntVect iv = bitEC();
                // DISCHARGE -- QwX
                //if (IM(iv,0) > 0.0) {  // Need an EC IM
                    out_water_flux_x_tot[iv[0]]     += Qwater_ecFab(iv, 0) * m_amrDx[0][1];                          
                    out_water_flux_x_chan[iv[0]]    += Qwater_ecFab(iv, 0) * m_amrDx[0][1] * CD_ecFab(iv, 0);        
            ...
            for (bit.begin(); bit.ok(); ++bit) {
                ...
                if (IM(iv,0) > 0.0) {
                    if (m_suhmoParm->m_n_moulins > 0) {
                        out_recharge_ext[iv[0]]      += (MS(iv, 0) * ramp + m_suhmoParm->m_distributed_input) * m_amrDx[0][1] * m_amrDx[0][0];  
                    } else {
                        out_recharge_ext[iv[0]]      += MS(iv, 0) * m_amrDx[0][1] * m_amrDx[0][0];  
                    }
                    out_recharge_MR[iv[0]]   += (MR(iv, 0)/m_suhmoParm->m_rho_w) * m_amrDx[0][1] * m_amrDx[0][0];
                    water_vol[iv[0]]         += GH(iv, 0)* m_amrDx[0][1] * m_amrDx[0][0];
                    Ylength[iv[0]]           += m_amrDx[0][1];
                }

                // Pressures
                if ((Pressi(iv,0) > 0.0) and (IM(iv,0) > 0.0)) {
       ...
       result = MPI_Allreduce(&out_recharge_ext[0], &recvMoulin[0], DomSize, MPI_CH_REAL,
                                  MPI_SUM, Chombo_MPI::comm);
       if (result != MPI_SUCCESS)
       {
           MayDay::Error("communication error on MPI_SUM");
       }
       out_recharge_ext = recvMoulin;

       Vector<Real> recvMoulinTot(DomSize);
       result = MPI_Allreduce(&out_recharge_MR[0], &recvMoulinTot[0], DomSize, MPI_CH_REAL,
                                  MPI_SUM, Chombo_MPI::comm);
       if (result != MPI_SUCCESS)
       {
           MayDay::Error("communication error on MPI_SUM");
       }
       out_recharge_MR = recvMoulinTot;
       ...
       xloc_bandHi_lo = recvMin;
        ...
        for (int xi = (DomSize - 2); xi > -1 ; xi--) {
            out_recharge_ext[xi]  = out_recharge_ext[xi]  + out_recharge_ext[xi+1] ;
            out_recharge_MR[xi]  = out_recharge_MR[xi]  + out_recharge_MR[xi+1] ;
        ...
        if (m_post_proc_shmip_temporal) {
            if (time_tmp % 86400 == 0) {
                ...
                pout() << "Time(h - d) rech dis = " <<  (m_time + a_dt -m_restart_time)/3600.  << " " << (m_time + a_dt -m_restart_time)/86400 
                                                                   << " " << out_recharge_ext[1] + out_recharge_MR[1] 
        ...
        //           << " " <<  time_tmp / 86400.  << " " <<  out_recharge_MR[0]  << " "  << out_recharge_ext[0] 
        //           << " " <<  out_recharge_ext[idx_bandMin_lo] << " " << out_recharge_ext[idx_bandMin_hi]
        //           << endl;
        //}
        ...
        if (m_post_proc_shmip) {
            ...
            for (int xi = 0; xi < DomSize; xi++) {
                Real x_loc = (xi+0.5)*m_amrDx[0][0];    
                       ...
                       << " " << out_recharge_ext[xi] << " " << out_recharge_MR[xi] << " " << avPressure[xi]/std::max(dom_sizeY[xi], 1.0)/1e6  
                       << endl;
            }
...
AmrHydro::regrid()
{
    ...
    if (m_max_level > 0) {
        ...
        for (int lev = m_regrid_lbase + 1; lev <= new_finest_level; ++lev) {
            ...
            m_iceMask[lev]         = destructiveRegrid(m_iceMask[lev], newDBL, m_iceMask[lev-1], m_refinement_ratios[lev-1]);
            ...
            // need special treatment for read vars -m_magVel and m_bedelevation and m_iceheight.. also m_iceMask ! 
            RealVect levelDx = m_amrDx[lev] * RealVect::Unit;
            ...
            if (lev > 0) {
                ...
                PiecewiseLinearFillPatch ghostFiller (newDBL, 
                                                      (m_bedelevation[lev-1])->disjointBoxLayout() ,  
                                                      (m_bedelevation[lev-1])->nComp(), 
                                                      (m_bedelevation[lev-1])->disjointBoxLayout().physDomain(), 
                                                      m_refinement_ratios[lev-1],
        ...
        //for (int lev = new_finest_level; lev > 0; lev--) {
        //    CoarseAverage averager(m_amrGrids[lev], 1, m_refinement_ratios[lev - 1]);
        //    averager.averageToCoarse(*m_iceMask[lev - 1], *m_iceMask[lev]);
        //}

        //for (int lev = 0; lev <= new_finest_level; ++lev) {
            //RealVect levelDx = m_amrDx[lev] * RealVect::Unit;
            //m_IBCPtr->correct_iceMask(levelDx, *m_overburdenpress[lev], *m_iceMask[lev]);
            //if (lev > 0) {
            //    // handle ghost cells on the coarse-fine interface
...
AmrHydro::tagCellsLevel(IntVectSet& a_tags, int a_level, int a_tag)
{
    ...
    LevelData<FArrayBox> levelGradPhi(m_amrGrids[a_level], SpaceDim, IntVect::Unit);
    ...
    } else if (m_tag_var[a_tag] == "IM") { 
        compute_grad_var(a_level, m_iceMask, levelGradPhi); 
        dit = m_overburdenpress[a_level]->dataIterator();
        for (dit.begin(); dit.ok(); ++dit) {
            FArrayBox& phi      = levelPhi[dit];
            FArrayBox& gradPhi  = levelGradPhi[dit];
            phi.copy(gradPhi, 0, 0, 1);
        }
...
AmrHydro::interpVelData() {
    ...
    // first create finest level -- Ref ratio should be 6 to go from 150km -> 25km for the thk and topg
    Vector<Box> finestBoxes;
    domainSplit(m_amrDomains[m_max_level], finestBoxes, m_max_box_size, m_block_factor);
    ...
    RefCountedPtr<LevelData<FArrayBox>> a_thk         = RefCountedPtr<LevelData<FArrayBox>> ( new LevelData<FArrayBox>(finestGrids, nPhiComp, ZeroghostVect) );
    RefCountedPtr<LevelData<FArrayBox>> a_thk_coarse  = RefCountedPtr<LevelData<FArrayBox>> ( new LevelData<FArrayBox>(m_amrGrids[0], nPhiComp, ZeroghostVect) );
    RefCountedPtr<LevelData<FArrayBox>> a_topg        = RefCountedPtr<LevelData<FArrayBox>> ( new LevelData<FArrayBox>(finestGrids, nPhiComp, ZeroghostVect) );
    RefCountedPtr<LevelData<FArrayBox>> a_topg_coarse = RefCountedPtr<LevelData<FArrayBox>> ( new LevelData<FArrayBox>(m_amrGrids[0], nPhiComp, ZeroghostVect) );

    // fill coarse data
    //LevelData<FArrayBox>& VelMag_coarse  = *m_magVel[0];
    LevelData<FArrayBox>& thk_coarse   = *m_iceheight[0];
    LevelData<FArrayBox>& topg_coarse  = *m_bedelevation[0];
    DataIterator dit = m_amrGrids[0].dataIterator();
    for (dit.begin(); dit.ok(); ++dit) {
        //FArrayBox& VM_coarse_dit    = VelMag_coarse[dit];
        //FArrayBox& a_V_coarse_dit   = (*a_V_coarse)[dit];
        FArrayBox& thk_coarse_dit    = thk_coarse[dit];
        FArrayBox& a_thk_coarse_dit   = (*a_thk_coarse)[dit];
        FArrayBox& topg_coarse_dit    = topg_coarse[dit];
        FArrayBox& a_topg_coarse_dit   = (*a_topg_coarse)[dit];
        //BoxIterator bit(a_V_coarse_dit.box());
        BoxIterator bit(a_thk_coarse_dit.box());
        for (bit.begin(); bit.ok(); ++bit) {
            IntVect iv = bit();
            //a_V_coarse_dit(iv,0)   = VM_coarse_dit(iv,0);
            a_thk_coarse_dit(iv,0)   = thk_coarse_dit(iv,0);
            a_topg_coarse_dit(iv,0)  = topg_coarse_dit(iv,0);
        }
    }
    ...
    interpolator.interpToFine(*a_thk, *a_thk_coarse);
    interpolator.interpToFine(*a_topg, *a_topg_coarse);
    ...
    stuffToPlot[0][0]  = new LevelData<FArrayBox>(m_amrGrids[0], 1, IntVect::Zero);  // dummy coarse
    stuffToPlot[0][1]  = new LevelData<FArrayBox>(finestGrids, 1, IntVect::Zero);    // dummy fine
    stuffToPlot[1][0]  = new LevelData<FArrayBox>(m_amrGrids[0], 1, IntVect::Zero);  // dummy coarse
    stuffToPlot[1][1]  = new LevelData<FArrayBox>(finestGrids, 1, IntVect::Zero);    // dummy fine

    LevelData<FArrayBox>& levelDummy      = *a_thk_coarse;
    LevelData<FArrayBox>& levelDummy2     = *a_topg_coarse;
    LevelData<FArrayBox>& levelDummySTP   = *stuffToPlot[0][0];
    LevelData<FArrayBox>& levelDummy2STP  = *stuffToPlot[1][0];
    LevelData<FArrayBox>& levelDummyF     = *a_thk;
    LevelData<FArrayBox>& levelDummy2F    = *a_topg;
    LevelData<FArrayBox>& levelDummyFSTP  = *stuffToPlot[0][1];
    LevelData<FArrayBox>& levelDummy2FSTP = *stuffToPlot[1][1];
    dit = m_amrGrids[0].dataIterator();
    for (dit.begin(); dit.ok(); ++dit) {
        levelDummySTP[dit].copy(levelDummy[dit], 0, 0, 1);
        levelDummy2STP[dit].copy(levelDummy2[dit], 0, 0, 1);
    }
    dit = finestGrids.dataIterator();
    for (dit.begin(); dit.ok(); ++dit) {
        levelDummyFSTP[dit].copy(levelDummyF[dit], 0, 0, 1);
        levelDummy2FSTP[dit].copy(levelDummy2F[dit], 0, 0, 1);
    }
                     ...
                     string namePlot)
{
    ...
    WriteAMRHierarchyHDF5(
        filename, m_amrGrids, plotData, vectName, domain, m_amrDx[m_max_level][0], dt, time(), m_refinement_ratios, numLevels);
...
AmrHydro::writeCheckpointFile() const
{
    ...
    // iceMask
    sprintf(compStr, "component_%04d", nComp);
    header.m_string[compStr] = iceMaskName;
    ...
    for (int lev = 0; lev <= m_max_level; lev++) {
        ...
        if (lev <= m_finest_level) {
            ...
            write(handle, *m_iceMask[lev], "iceMaskData", m_iceMask[lev]->ghostVect());
        } // end loop over levels
    }
...
AmrHydro::restart(string& a_restart_file)
{
    ...
    for (int lev = 0; lev <= m_finest_level; lev++) {
        ...
        //                       *m_bumpHeight[lev],
        //                       *m_bumpSpacing[lev]);
        m_IBCPtr->resetCovered(levelDx, *m_suhmoParm, *m_head[lev], *m_iceheight[lev], *m_overburdenpress[lev]);
        //m_IBCPtr->setup_iceMask(*m_overburdenpress[lev], *m_iceMask[lev]);
    }
    //for (int lev = m_finest_level; lev >  0; lev--) {
    //    RealVect levelDx = m_amrDx[lev] * RealVect::Unit;
