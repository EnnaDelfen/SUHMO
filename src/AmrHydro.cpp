                          ...
                          Vector<LevelData<FArrayBox>*>& a_RHS)
{
    ...
    int numSmooth = 6;  // number of relax before averaging
    int numBottom = 10;  // num of bottom smoothings
...
AmrHydro::timeStep(Real a_dt)
{
    ...
    while (!converged_h) { 
        ...
        for (int lev = 0; lev <= m_finest_level; lev++) {
            ...
            if (lev > 0) {
                int nGhost = levelcurRe.ghostVect()[0];
                PiecewiseLinearFillPatch headFiller(m_amrGrids[lev],
                                                    m_amrGrids[lev - 1],
                                                    1, // ncomps
                                                    m_amrDomains[lev - 1],
        ...
        for (int lev = 0; lev <= m_finest_level; lev++) {
            ...
            // handle ghost cells on the coarse-fine interface
            if (lev > 0) {
                int nGhost = levelmR.ghostVect()[0];
                PiecewiseLinearFillPatch headFiller(m_amrGrids[lev],
                                                    m_amrGrids[lev - 1],
                                                    1, // ncomps
                                                    m_amrDomains[lev - 1],
   ...
   if (m_verbosity > 3) {
    ...
    // handle ghost cells on the coarse-fine interface
    for (int lev = 1; lev <= m_finest_level; lev++) {
        int nGhost = (*m_gapheight[lev]).ghostVect()[0];
        PiecewiseLinearFillPatch headFiller(m_amrGrids[lev],
                                            m_amrGrids[lev - 1],
                                            1, // ncomps
                                            m_amrDomains[lev - 1],
   ...
   if (m_verbosity > 3) {
    ...
    for (int lev = 0; lev <= m_finest_level; lev++) {
        ...
        for (dit.begin(); dit.ok(); ++dit) {
            ...
            for (bit.begin(); bit.ok(); ++bit) {
                IntVect iv = bit(); 
                //newB(iv,0) = std::max(RHS(iv,0) * a_dt + oldB(iv,0), 1.0e-6);
                newB(iv,0) = RHS(iv,0) * a_dt + oldB(iv,0);
            }
        }
    }  // loop on levs
